...
class AGV:
    _all_instances = set()
    def __init__(self, id, current_node, graph, graph_processor, cost = 0, version_of_graph = -1):
        self.id = id
        ...
        if current_node not in self.graph.nodes.keys():
            #pdb.set_trace()
            node = self.graph_processor.find_node(current_node)
